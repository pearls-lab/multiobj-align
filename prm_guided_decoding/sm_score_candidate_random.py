...
def process_worker(args_tuple):
    ...
    chunk_data, model_name, max_length, batch_size, device_id, worker_id, seed = (
        args_tuple
    )
    ...
    if seed is not None:
        random.seed(seed + worker_id)  # Different seed per worker
    ...
    for row_data in chunk_data:
        ...
        random_score = float(scores[random_idx])
        random_candidate = candidates[random_idx]

        out_obj = {
            **row,
            "rm_model": model_name,
            "scores": scores,
            "best_index": random_idx,  # Keep same field name for compatibility
            "best_score": random_score,  # Keep same field name for compatibility
            "best_candidate": random_candidate,  # Keep same field name for compatibility
        }
        results.append(out_obj)

        print(
            f"[Worker {worker_id}] Processed id={row.get('id', '')} random_idx={random_idx} random_score={random_score:.4f}"
        )
...
def process_file_parallel(
    ...
    for i, chunk in enumerate(chunks):
        if chunk:  # Only create worker if chunk has data
            worker_args.append((chunk, model_name, max_length, batch_size, i, i, seed))
                ...
                try:
                    ...
                    for result in results:
                        ...
                        print(
                            ...
                            f"random_idx={result['best_index']} random_score={result['best_score']:.4f} "
                            f"id={result.get('id', '')}"
                        )
    ...
    print(
        f"Completed! Randomly selected candidates for {completed_count} rows → {output_path}"
    )
...
def process_file_single_model_parallel(
    ...
    with output_path.open("w", encoding="utf-8") as out_f:
        for i, (messages, candidates, row) in enumerate(data):
            ...
            print(
                f"[Completed {written}/{len(data)}] random_idx={random_idx} random_score={random_score:.4f} id={row.get('id', '')}"
            )

    print(f"Completed! Randomly selected candidates for {written} rows → {output_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Score preference candidates and select RANDOM candidate (not best) using multiple GPUs"
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Output JSONL with scores and randomly selected candidate",
    )
    parser.add_argument(
        "--model",
        type=str,
