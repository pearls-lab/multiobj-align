"""
Enhanced inference script for running multi-head DPO models on MATH-500 dataset.
Supports base model, SFT model, and DPO model with specialized heads or ensemble generation.
Now supports processing data in splits for parallel execution.
"""
...
def run_inference_on_sample(
    ...
    # Generation parameters - match math_base.py settings
    generation_kwargs = {
...
def load_model(
    ...
    if model_type in ["base", "sft"]:
        ...
        # Load DPO checkpoint
        if checkpoint_path:
            # Handle different checkpoint sources with support for directories
            local_checkpoint_path = None
...
def run_sm_acc_inference(
    ...
    for idx, example in enumerate(tqdm.tqdm(examples, disable=disable_tqdm)):
        ...
        # Create output record - adapted for sm_acc format
        output_record = {
            "id": unique_id,
            "question": question,
