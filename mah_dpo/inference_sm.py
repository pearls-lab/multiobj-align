"""
Inference script for running multi-head DPO models on Socratic Mind dataset.
...
# Try to import MultiHeadCausalLM, but make it optional for base model usage
try:
    ...
    logger.warning(
        "multihead_model.MultiHeadCausalLM not available - DPO model features disabled"
    )
...
def download_from_s3(s3_path: str, local_cache_dir: Optional[str] = None) -> str:
    ...
    if not BOTO3_AVAILABLE:
        raise ImportError("boto3 not available. Install boto3 for S3 support.")
...
def download_from_hf_hub(
    ...
    if not HF_HUB_AVAILABLE:
        raise ImportError(
            "huggingface_hub not available. Install huggingface_hub for HF Hub support."
        )
...
def clean_assistant_response(response_text: str) -> str:
    """Trim the assistant response to only the first assistant turn.

    - Removes a leading "Assistant:" marker if present
    - Cuts off at the first occurrence of any other role marker like
      "Student:", "Human:", "User:", or another "Assistant:" (new turn)
    """
    response = response_text.strip()

    # Remove any leading assistant marker
    leading_markers = ["Assistant:", "assistant:", "ASSISTANT:"]
    for marker in leading_markers:
        if response.startswith(marker):
            response = response[len(marker) :].lstrip()
            break

    # Markers that indicate a new turn/speaker
...
def load_sm_acc_dataset(
    data_file: str = "sm_acc.json",
    ...
    """
    Load the sm_acc.json dataset with optional subsampling and split processing.
    ...
    Returns:
        List of examples with question, chosen, rejected, and metadata.
    """
    ...
    logger.info(f"Found {total_samples} samples in dataset")
...
def run_inference_on_sample(
    ...
    """
    Run inference on a single conversation sample with controlled randomness.
    """
    ...
    # Handle conversation formatting
    if system_prompt:
        ...
        # Use the conversation as-is since question already contains the full context
        # The question field already contains the conversation ending with "Assistant:"
    ...
    # Ensure sampling is enabled for randomness
    generation_kwargs = {
...
def load_model(
    ...
    if model_type in ["base", "sft"]:
        ...
        if model_type == "sft" and checkpoint_path:
            # Handle different checkpoint sources
        ...
        if MultiHeadCausalLM is None:
            raise ImportError(
                "MultiHeadCausalLM not available. Install multihead_model module for DPO support."
            )
...
def run_sm_acc_inference(
    ...
    """
    Run inference on sm_acc.json dataset and save results to a JSONL file.
    ...
    Args:
        ...
        num_candidates: Number of candidate responses to generate per question
    ...
    for idx, example in enumerate(tqdm.tqdm(examples, disable=disable_tqdm)):
        ...
        unique_id = metadata.get("original_id", f"sample_{idx}")

        # Generate multiple candidates for this sample
        candidates = []
        for candidate_idx in range(num_candidates):
            # Run inference for each candidate with different seed offset
            responses = run_inference_on_sample(
                ...
                sample_index=idx * num_candidates + candidate_idx,
            )
            ...
            candidates.append(
                {
                    ...
                    "generation_seed": (
                        generation_seed + (idx * num_candidates + candidate_idx)
                        if generation_seed is not None
                        else None
                    ),
                }
        ...
        if (idx + 1) % 10 == 0 or idx == 0:
            ...
            if idx == 0:
                ...
                logger.info(
                    f"Chosen: {chosen[:100]}..." if len(chosen) > 100 else chosen
                )
                logger.info(
                    f"Rejected: {rejected[:100]}..."
                    if len(rejected) > 100
                    else rejected
                )
                logger.info(f"----- Sample Candidates ({num_candidates} total) -----")
                for candidate in candidates:
                    candidate_idx = candidate["candidate_index"]
                    response = candidate["response"]
                    logger.info(
                        f"Candidate {candidate_idx}: {response[:200]}..."
                        if len(response) > 200
                        else response
                    )
                logger.info("---------------------------\n")
...
def main():
    parser = argparse.ArgumentParser(description="Run inference on sm_acc.json dataset")
    ...
    parser.add_argument(
        ...
        default="base",
        help="Type of model to use (default: base)",
    )
    parser.add_argument(
        "--model_name",
        type=str,
        default="Qwen/Qwen2.5-7B-Instruct",
        help="Name or path of the base model (default: Qwen/Qwen2.5-7B-Instruct)",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="sm_acc.json",
        help="Path to the JSON data file (default: sm_acc.json)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of candidate responses to generate per question (default: 1)",
    )
    parser.add_argument(
        "--output_file",
        type=str,
        default="inference_results.jsonl",
        help="Output file path (JSONL format, default: inference_results.jsonl)",
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    # Update the run_sm_acc_inference call to include split parameters and top_k
    run_sm_acc_inference(
